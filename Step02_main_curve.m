...
printerInput = validUserInput('Select the printer (Raise3D = 1, AON M2 = 2, Aerotech Gantry = 3) : ',0 ,'1','2','3');

% Opening the Gcode file
nom_fic_in = input('File name of the GCODE to curve (no extension required) : ','s');
fid=fopen([nom_fic_in '.gcode']);
while fid<0
    nom_fic_in = input('File does not exist\nFile name of the GCODE to curve (no extension required) : ','s');
    fid=fopen([nom_fic_in '.gcode']);
end

% User choice for radius and part dimensions
infosInput = validUserInput('Do you want to curve a flattened Gcode toolpath after using the script "Step01_main_flatten" (Y/N)? ',0 ,'Y','N');
